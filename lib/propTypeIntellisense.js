...
const propTypeIntellisense = () => {
  ...
  const propTypes = props[1].replace(/.PropTypes./g,'').split(',').map( p => p.replace(/\r/g, '').replace(/\n/g, '') );
  
  const reactClassName = sourceCode.substring(sourceCode.substring(0, props.index).lastIndexOf('\n'), props.index).replace(/\r/g, '').replace(/\n/g, '');

  const argumentString = "/**\r\n* @arguments {Component<{" + propTypes.join() + ">}\r\n*/\r\n";
  const snippet = new vscode.SnippetString(argumentString);
